...
const RListed = () => {
  ...
  useEffect(() => {
    const main = async () => {
      const url = `${server}/api/blockchain/recentlylisted`;
          ...
          {data ? (
            data.length > 0 ? (
              data.map((nft, i) => (
                <Link
                  key={i}
                  href={`/collection/${nft.collectionAddress}/${nft.tokenId}`}
                >
                  <a>
              ...
              <div className={styles.notActive}>
                No recent listings in the last hours
              </div>
            )
          ) : (
...
export default React.memo(RListed);
